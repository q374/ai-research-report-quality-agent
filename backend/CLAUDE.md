...
Shadow execution is controlled by `EvidenceValidationConfig` in `packages/harness/deerflow/config/evidence_validation_config.py`. It is disabled by default and requires both an allowed user ID and an allowed quality-profile ID; `config.example.yaml` must keep `allowed_user_ids` empty. Real allowed user IDs belong only in ignored local `config.yaml` or an approved secret/configuration system.

`app/gateway/evidence_validation/` owns collection, application orchestration, and post-run dispatch. The collector trusts persisted run/event fields for answer text, model, Token, tool names, and URLs; it must not trust model self-reports or persist complete tool arguments. Automatic dispatch requires a successful run whose metadata contains both an allowed `quality_profile_id` and `research_brief`. Manual replay reads an existing successful run and must never start an Agent, call a model/tool, or mutate run metadata. Shadow failures are isolated and must not rewrite the original run status or answer.

The protected APIs are `GET /api/threads/{thread_id}/runs/{run_id}/evidence-validation` and `POST /api/threads/{thread_id}/runs/{run_id}/evidence-validation/replay`. Both verify run ownership and thread binding; replay additionally requires the local user/profile allowlists. Ordinary DeerFlow answers do not reliably contain structured Claim/Evidence, so missing semantic inputs must be stored as `semantic_evaluation=not_evaluable`, never as a semantic pass.

Persistent validation records use `EvidenceValidationRepository` and the `evidence_validations` table introduced by Alembic revision `0003_evidence_validations`. The `(run_id, report_hash)` uniqueness constraint is the idempotency boundary; legacy bootstrap fixtures must not pre-create this post-baseline table. The feature is unavailable when application persistence uses the memory backend because memory mode cannot guarantee durable, idempotent records.

Manual evidence reviews are added by Alembic revision `0004_evidence_reviews`. `final_status` and the append-only `review_decisions_json` audit list stay on the exact validation row, and each decision is bound to `validation_id`, `report_hash`, reviewer, timestamp, and idempotency key. The protected review endpoint permits the owning user only; approval is valid only for `review_required`, while return/reject require a reason. A new report hash produces a new validation row, so old approval never transfers implicitly.

Evidence-profile runs also require `run_events.backend: db` in the ignored local `config.yaml`. This field is startup-only: restart Gateway after changing it, and verify that `app.state.run_event_store` is a `DbRunEventStore`. If a profile is enabled while events remain non-persistent, keep the user-visible answer but record `non_persistent_event_store` and fail closed for semantic evaluation; never infer that zero observed tools means zero actual tools.

`submit_evidence_report` is the structured completion contract for allowed evidence profiles. It returns a ToolMessage; `EvidenceReportFinalizerMiddleware` deterministically appends the final AI message after the direct-return exit, so validation does not cause a second model call. Persisted `evidence.report.submitted` and final `ai_message` events must share `message_id`; the stored validation record must bind that message to its computed `report_hash`.

Keep the three authority layers separate: system-observed event facts, Agent-proposed Claim/Evidence relations, and human review decisions. Automatic validation may produce only `blocked` or `review_required`; it must never create `confirmed`. Ordinary chats, non-allowlisted users, and runs without the allowed quality profile must retain the original behavior.
