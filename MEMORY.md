...
## 2026-09-20 GitHub 项目首页作品集化

- 已将根目录 `README.md` 从 DeerFlow 原版介绍改为面向招聘方的中文项目主页，突出产品定位、真实评测数据、四层质量机制、目标用户、验证结果、三分钟评审路径、个人贡献和真实性边界。
- DeerFlow 原版说明完整保留为 `README_DEERFLOW_UPSTREAM.md`，并在新首页明确上游来源、复用能力、基线提交和 MIT License，避免把开源基础包装成个人从零开发。
- 首页复用已提交的阶段三验收截图，并提供统一 PRD、AI 产品经理案例、联合验收、实际账号界面验收、质量规则和校验器规格的直接入口。
- 本轮只改文档，未改产品代码、未运行模型、未增加 API 费用。相对链接检查、原版 README 文本保留核对和 Git diff 检查通过。
