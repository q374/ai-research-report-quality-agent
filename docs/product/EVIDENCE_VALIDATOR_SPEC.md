...
版本：v1.0
更新时间：2026-09-19
阶段：阶段 A 离线确定性原型已实现；尚未接入 DeerFlow
...
|---|---|---|---|---|
| EV-01 | 当前性 | 当前结论是否只依赖历史发布页；标为 `current_fact` 的结论是否仍含“首发、首批、当时、发布时”等明确历史叙述；历史内容是否标时 | 阻断 | 改成 `historical_fact` 并标时，或补当前来源 |
...
| EV-10 | 审计完整性 | 模型、Prompt 版本、访问时间、Token、费用、延迟和复核结论是否齐全 | 阻断（必填缺失） | 补齐运行记录；契约不完整时 fail-closed |

### 为什么首版不用第二个模型评分

第二个模型可能重复产生幻觉，还会增加成本和延迟。首版先处理可以确定计算的问题：字符数、来源数、必填字段、状态、URL、显式否定词和证据摘录反例。无法确定的语义问题交给人工，而不是伪装成自动正确。

EV-01 的历史叙述检查只识别明确时间语义，不根据来源发布日期武断认定事实已经失效；“旧页面中的内容现在是否仍成立”继续交给人工复核或当前来源证明。
...
## 15. 阶段 A 实现状态（2026-09-19）

- 已实现独立标准库脚本 `scripts/evidence_validator.py`，不依赖第二个模型或新增第三方包。
- 已固定 T001 v1—v3 的结构化离线夹具，并生成可审计 JSON 结果。
- 7 项单元测试通过；三份已知 Badcase 均为 `blocked`，无错误放行。
- v3 实际正文由程序计为 877 字符，4 次页面查看经别名和锚点归一后为 2 个唯一页面。
